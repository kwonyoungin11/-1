...
**날짜:** 2026-07-09  
**Phase 6 스냅샷:** `feature/pw05-phase6` — 증거 매핑 갱신, **LIVE_READY=false 유지**
...
| Phase | 이름 | 돈 위험 | 통과 기준 (요약) | 현재 (2026-07-09) |
|-------|------|---------|------------------|-------------------|
| 0 | 하네스 | 없음 | build/test/scan, live 차단 | **완료** |
| 1 | 사용자 cockpit 설계 | 없음 | 오너가 화면 구조 승인 | **완료** (Avalonia 레이아웃 고정) |
| 2 | 토스 읽기 전용 연결 | 매우 낮음 | 계좌·시세 읽기 + 마스킹 | **코드 완료** · mock 기본 · live HTTP 옵션 · **실 스모크 로그 선택** |
| 3 | 신호 + 리스크 게이트 | 없음(후보만) | 위험 후보 block 테스트 | **골격 완료** (unit) |
| 4 | dry-run | 없음 | 파이프 안정·감사 로그 | **단위 완료** · 기간 운영 증거는 Phase 6에서 계속 |
| 5 | paper trading | 없음 | 기간 증거·오너 이해 | **ledger/unit 완료** · **다일 기간 증거 미완** |
| 6 | live readiness | 없음(아직 잠금) | 체크리스트 전부 증거 | **진행 중** — 자동화 PASS, A–E 사람/운영 증거 **미완** |
| 7 | 실거래 개방 (좁게) | 있음 | 소액·한도·kill switch | **금지** (오너 승인 전) |
...
- Phase 3–5: 골격 완료 (신호·리스크·dry-run·paper **단위 테스트 통과**)
- **Phase 6 (지금):**  
  ...
  - 체크리스트/증거 문서: [`docs/LIVE_READINESS_CHECKLIST.md`](../LIVE_READINESS_CHECKLIST.md) · [`docs/plans/LIVE_READINESS_EVIDENCE.md`](LIVE_READINESS_EVIDENCE.md)  
  - **실거래: 계속 차단** (오너 Phase 7 승인 전 개방 없음)
- **UI/UX:** 현재 차트+하단 조작 고정 · 플랜 진행은 백엔드/안전/토스 연결·증거

## Phase 6 정직한 갭 (live 전 필수)

| 갭 | 상태 |
|----|------|
| 기계적 차단 무결성 | **OK** (`LIVE_READY=false` 기대 통과) |
| dry-run / paper **단위** | **OK** |
| paper **운영 기간** 증거 | 미완 |
| 토스 **실** 읽기 스모크 (redacted) | 미완 (선택·오너) |
| 멱등/중복 가드 | 미완 |
| incident 리허설 날짜 | 미완 |
| 오너 live 승인서 | 없음 (의도) |
...
1. (선택) 오너 승인 후 `TOSS_ALLOW_LIVE_HTTP=true` 로 **읽기 전용** 실 API 1회 스모크 → redacted 로그를 `artifacts/`  
2. paper 운영 기간 합의 + ledger export (이익 보장 문구 금지, 안정 증거만)  
3. Phase 6 체크리스트 나머지 항목 채우기 — **여전히 실주문 잠금**  
4. 멱등/중복 가드 설계·테스트 (live submit 추가 없이)  
5. Phase 7은 오너 명시 승인 전까지 **진행 금지**
...
```text
오너 요약:
- 현재 단계: Phase 6 live readiness (증거 문서화) — Phase 7 아님
- 안전 상태: fail-closed 기본값 유지, 주문 submit 경로 없음
- live order 가능 여부: 불가능
- 오늘 해야 할 결정: paper 운영 기간 / (선택) 읽기 전용 실 API 스모크
- 내가 추천하는 선택: 실주문 계속 잠금, 읽기·연습 증거만 쌓기
- 이유: 돈 나가는 경로를 열 조건(체크리스트 A–E + 승인)이 아직 없음
